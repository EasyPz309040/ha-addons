...
The add-on adds a **Cluster Control** entry to the Home Assistant sidebar
(served through ingress, so it sits behind HA's own login and exposes no
port on the LAN).
